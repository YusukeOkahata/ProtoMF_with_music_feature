...
class ProtoRecDataset(data.Dataset):
    ...
    def load_data(self):
        ...
        # CSR行列を確認可能な形式に変換して保存する
        def save_csr_to_csv(csr_matrix, output_path):
            """
            ...
            """
            coo = csr_matrix.tocoo()  # COO形式に変換（行列の非ゼロ要素を取得しやすくする）
            df = pd.DataFrame({
                'user': coo.row,   
                'item': coo.col,   
                'label': coo.data 
        ...
        # CSVに保存
        output_csv_path = '/content/drive/MyDrive/Master/research/ProtoMF/rec_sys/train_csr_matrix.csv'
        save_csr_to_csv(train_csr, output_csv_path)
        print(f"(train)CSR行列をCSVに保存しました: {output_csv_path}")
        ...
        if self.split_set == 'val':
            ...
            # CSVに保存
            output_csv_path = '/content/drive/MyDrive/Master/research/ProtoMF/rec_sys/val_csr_matrix.csv'
            save_csr_to_csv(val_csr, output_csv_path)
            print(f"(val)CSR行列をCSVに保存しました: {output_csv_path}")
            ...
            # CSVに保存
            output_csv_path = '/content/drive/MyDrive/Master/research/ProtoMF/rec_sys/test_csr_matrix.csv'
            save_csr_to_csv(test_csr, output_csv_path)
            print(f"(test)CSR行列をCSVに保存しました: {output_csv_path}")
